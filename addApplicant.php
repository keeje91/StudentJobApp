	...
	<meta charset="utf-8">
      ...
      $(document).ready(function(){
          $('#stu-select').change(function() {
                      // Get the selected student's DEPT_CODE
                      var selectedDeptCode = $(this).find(':selected').data('dept');

                      // Update the job select options based on the selected student's DEPT_CODE
                      $.ajax({
                          url: 'get_jobs.php', // Update the URL to the file that fetches job options from database
                          type: 'POST',
                          data: {state_code: selectedDeptCode},
...
<div class = "center">
<h1>Add Application By State</h1>
<form name="insertForm" action="addApplicant.php" method="post">
          <div class="inputbox">
                 <select id= "stu-select" name="STUDENT_ID" style="position: absolute;top: 0;left: 0;width: 100%;border: 2px solid #000;outline: none;background: none;padding: 10px;border-radius: 10px;font-size: 1.2em;" required>
                       ...
                       <?php
                        ...
                        $sqlDept = "SELECT * FROM STUDENTS";
                        $resultDept = mysqli_query($conn, $sqlDept);

                        while ($row = mysqli_fetch_assoc($resultDept)) { ?>
                              <option value="<?php echo $row['STUDENT_ID']; ?>" data-dept="<?php echo $row['STATE_NAME']; ?>"><?php echo $row['STUDENT_ID']; ?></option>
                          <?php } ?>
                     </select>
          </div>
